...
class NEField(Field):
    ...
    def build(self, dataset, min_freq=1):
        counter = Counter(i
                          for ne in getattr(dataset, self.name)
                          for row in self.preprocess(ne)
                          for i in row if i is not None)

        self.vocab = Vocab(counter, min_freq, self.specials, self.unk_index)
        self.vocab.extend(["NON-NE",]) # NON-NE
        # import pdb
        # pdb.set_trace()
    def transform(self, nes):
        nes = [self.preprocess(ne) for ne in nes]
        if self.use_vocab:
            nes = [[[self.vocab[i] if i is not None else -1 for i in row] for row in ne] for ne in nes]
        if self.bos:
            nes = [[[self.bos_index]*len(ne[0])] + ne for ne in nes]
        if self.eos:
            nes = [ne + [[self.eos_index]*len(ne[0])] for ne in nes]

        nes = [torch.tensor(ne) for ne in nes]
        return nes